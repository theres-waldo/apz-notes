...
**How to avoid clobber builds deleting the clangd index**

Background:

* `./mach build-backend -b Clangd` (also called by `./mach ide vscode`) generates a compilation database (`compile_commands.json`) for clangd's use, in `<objdir>/clangd`.
  * For vscode users, clangd is pointed to the compilation database by adding `"--compile-commands-dir=<objdir>/clangd"` to `"clangd.arguments"` in `<srcdir>/.vscode/settings.json`.
* clangd places its project index next to the compilation database, in `<objdir>/clangd/.cache/clangd`.
* Clobber builds (done via `./mach clobber`, or `./mach build` if `AUTOCLOBBER=1` is set in your mozconfig), which are often required when pulling from upstream, delete the entire `<objdir>`, including therefore the clangd index.
* This is unfortunate because clangd could in fact safely reuse a significant part of its index even across clobber builds.

Current workaround:

* Copy the compilation database from `<objdir>/clangd` to a location outside of `<objdir>` (let's call it `<CDBdir>`).
* Point clangd to the new location by updating `"clangd.arguments"` in `<srcdir>/.vscode/settings.json` to have `"--compile-commands-dir=<CDBdir>"`.
* Note: you'll need to re-copy the compilation database from `<objdir>/clangd` to `<CDBDir>` every time you re-run `./mach build-backend -b Clangd` (which you should generally do after every pull to pick up compile commands for new files that may have been added).

Proper solution:

* The default location of the compilation database (as generated by `./mach build-backend -b Clangd`) should be configurable to be outside of `<objdir>` (or even outside of `<objdir>` by default). [Bug 1764209](https://bugzilla.mozilla.org/show_bug.cgi?id=1764209) tracks.
